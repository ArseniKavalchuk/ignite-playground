...
@SpringBootTest
public class IgniteExpireEvictTest {
    ...
    public void testEviction() throws Exception {
        ...
        conf
            ...
            //.setEvictionPolicy(new FifoEvictionPolicy<>(10_000_000))
            .setRebalanceMode(CacheRebalanceMode.NONE)
            .setIndexedTypes(Long.class, NumberEvent.class)
            .setStoreByValue(false);
        IgniteCache<Long, NumberEvent> cache = ignite
            .getOrCreateCache(conf)
            .withExpiryPolicy(new CreatedExpiryPolicy(new Duration(TimeUnit.HOURS, 1)))
            .withKeepBinary();
        Map<Long, NumberEvent> batch = new ConcurrentSkipListMap<>();
        SqlFieldsQuery q1 = new SqlFieldsQuery("select count(1) from \"evict_test\".NumberEvent");
        SqlFieldsQuery q2 = new SqlFieldsQuery("select count(1) from \"evict_test\".NumberEvent where topic = 'Topic 4'");
